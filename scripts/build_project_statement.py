...
def build_english():
    ...
    add_image(doc, "assets/documentation/screenshots/perform_actual.png", 6.0)
    add_caption(doc, "Actual Perform page: source control, camera feedback, serial effects, recording and master monitoring in one view.")
    ...
    # Pages 2–3 — actual system functions.
    page_heading = add_heading(doc, "1. System Functions", 1)
    page_heading.paragraph_format.page_break_before = True
    add_body(doc, "The finished interface is organised into five performance pages. Perform provides the live overview; Source, FX, Gesture and Master expose focused controls without changing the underlying signal or mapping state.", after=4)
    add_image(doc, "assets/documentation/screenshots/source_actual.png", 4.5)
    add_caption(doc, "Source page — microphone, file and granular inputs with a persistent Source Mixer and dry-bus output.")
    add_module(doc, "Source", "The selected source editor exposes its detailed controls while the mixer balances all three inputs. The shown Mic Input includes channel routing, a linked noise gate, output metering and direct access to Mic EQ.")
    add_image(doc, "assets/documentation/screenshots/fx_actual.png", 4.5)
    add_caption(doc, "FX page — one serial effect editor at a time with four persistent stage-output controls.")
    add_module(doc, "FX", "The screenshot shows Bitcrusher processing and live level activity. Each fader mirrors the existing Output Gain of its stage, so the overview and effect editor remain synchronized.")

    page_heading = add_heading(doc, "1. System Functions — continued", 1)
    page_heading.paragraph_format.page_break_before = True
    add_image(doc, "assets/documentation/screenshots/gesture_actual.png", 4.85)
    add_caption(doc, "Gesture page — live hand landmarks, spatial work zones, recognized target and mapped numerical values.")
    add_module(doc, "Gesture", "The camera image separates target selection from parameter adjustment. Finger counts 1–5 select processing targets, a fist holds the current values, and the panel confirms gesture, target, X/Y/pinch input and mapped parameters.")
    add_image(doc, "assets/documentation/screenshots/master_actual.png", 4.85)
    add_caption(doc, "Master page — record, edit, preview and WAV export beside an independent final-listening monitor.")
    add_module(doc, "Master", "The post-FX stereo signal enters a non-destructive editor with range reset, preview, new-take and 24-bit WAV export controls. Monitor level and mute remain separate from the exported recording.")

    # Page 4 — motivation and interaction.
    page_heading = add_heading(doc, "2. Artistic Motivation", 1)
    page_heading.paragraph_format.page_break_before = True
    ...
    add_heading(doc, "3. Interaction Design", 1)
    ...
    page_heading = add_heading(doc, "4. Technical Realisation", 1)
    page_heading.paragraph_format.page_break_before = True
    ...
    # Page 6 — sonic intent, reflection and current stage.
    page_heading = add_heading(doc, "5. Sound Design Strategy", 1)
    page_heading.paragraph_format.page_break_before = True
    ...
    add_heading(doc, "6. Reflection and Current Stage", 1)
...
def build_chinese():
    ...
    add_image(doc, "assets/documentation/screenshots/perform_actual.png", 6.0)
    add_caption(doc, "实际 Perform 页面：在同一视图中呈现音源、摄像头反馈、串联效果器、录制与主监听。")
    ...
    page_heading = add_heading(doc, "1. 系统功能导览", 1)
    page_heading.paragraph_format.page_break_before = True
    add_body(doc, "完成后的界面由五个表演页面构成。Perform 提供现场总览；Source、FX、Gesture 与 Master 在不改变底层音频和映射状态的前提下，分别呈现更集中的控制。", after=4)
    add_image(doc, "assets/documentation/screenshots/source_actual.png", 4.5)
    add_caption(doc, "Source 页面——麦克风、文件与粒子音源，以及固定显示的 Source Mixer 和干声总线。")
    add_module(doc, "Source", "左侧显示当前选中音源的详细控制，右侧混合三个输入。图中的 Mic Input 包含声道分配、联动噪声门、输出电平显示，以及直接打开 Mic EQ 的入口。")
    add_image(doc, "assets/documentation/screenshots/fx_actual.png", 4.5)
    add_caption(doc, "FX 页面——一次编辑一个串联效果器，同时固定显示四个阶段的输出控制。")
    add_module(doc, "FX", "截图展示了 Bitcrusher 的处理与实时电平。每个推子都镜像对应阶段已有的 Output Gain，因此总览与插件编辑器始终保持同步。")

    page_heading = add_heading(doc, "1. 系统功能导览（续）", 1)
    page_heading.paragraph_format.page_break_before = True
    add_image(doc, "assets/documentation/screenshots/gesture_actual.png", 4.85)
    add_caption(doc, "Gesture 页面——实时手部关键点、空间工作区、识别目标与数值化映射结果。")
    add_module(doc, "Gesture", "摄像头画面把目标选择与参数调整分开。手指数 1–5 选择控制目标，握拳保持当前数值；右侧面板同时确认手势、目标、X/Y/Pinch 输入与实际映射参数。")
    add_image(doc, "assets/documentation/screenshots/master_actual.png", 4.85)
    add_caption(doc, "Master 页面——录制、编辑、试听与 WAV 导出，并配有独立的最终监听。")
    add_module(doc, "Master", "效果器之后的立体声信号进入无损编辑器，可重置范围、试听、创建新 Take 并导出 24-bit WAV；监听电平与静音不会改变导出的录音。")

    page_heading = add_heading(doc, "2. 艺术动机", 1)
    page_heading.paragraph_format.page_break_before = True
    ...
    page_heading = add_heading(doc, "4. 技术实现", 1)
    page_heading.paragraph_format.page_break_before = True
    flow = doc.add_paragraph()
    flow.alignment = WD_ALIGN_PARAGRAPH.CENTER
    flow.paragraph_format.space_before = Pt(0)
    ...
    page_heading = add_heading(doc, "5. 声音设计策略", 1)
    page_heading.paragraph_format.page_break_before = True
